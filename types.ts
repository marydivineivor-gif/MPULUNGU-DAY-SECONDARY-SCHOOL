...
export interface FeePayment {
  ...
  studentId: string;
  amount: number;
  category: 'Tuition' | 'PTA' | 'Uniform' | 'Other';
  date: string;
  term: string;
  recordedBy: string;
}
...
export interface TeacherAttendance {
  ...
  status: 'Reported' | 'Absent' | 'Late' | 'On Leave';
  recordedBy: string; // Admin or HoD ID
}

export interface TeachingFileRecord {
  id: string;
  type: string; // WORK_PLAN, LESSON_PLAN, etc.
  teacherId: string;
  data: any; // JSON data for the template
  status: 'Draft' | 'Submitted to HOD' | 'Submitted to Admin' | 'Approved' | 'Returned';
  hodComment?: string;
  adminComment?: string;
  isTaught?: boolean; // For Lesson Plans
  createdAt: string;
  updatedAt: string;
}
